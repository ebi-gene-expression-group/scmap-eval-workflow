...
### directories. It also allows to extract any metadata (inferred cell types) from reference experiment    
...
files = c(opt$matrix_file_url, opt$barcodes_file_url, opt$gene_id_file_url)
names = c("matrix.mtx", "barcodes.tsv",  "genes.tsv")
for(idx in 1:length(files)){
    d = paste0(opt$output_dir_path, names[idx])
    # download file
    download.file(url = files[idx], destfile=d)
    # gunzip, if necessary
...
if(!is.na(opt$metadata_file_url)){
    ...
    if(summary(file("reference_metadata.txt"))$class == "gzfile") gunzip(d)
